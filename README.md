...
화자는 `speakers` 맵으로 색이 정해짐: 김우중 하늘색, 최윤정 분홍, 동시 흰색, 대표님 연한 민트, 김예지 연한 라벤더, 김형인 본부장 연한 크림, 상장 낭독 금색. 대본을 통째로 교체할 때는 `version` 값을 올릴 것. 브라우저에 저장된 이전 수정본이 새 대사에 잘못 붙는 것을 막아줌.
